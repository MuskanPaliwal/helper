...
export const ConversationListItem = React.memo(
  ({ conversation, isActive, onSelectConversation, isSelected, onToggleSelect }: ConversationListItemProps) => {
    const listItemRef = useRef<HTMLAnchorElement>(null);
    const { searchParams } = useConversationsListInput();

    // Memoize search terms computation
        ...
        >
          <div className="flex items-start gap-4 px-2 md:px-4">
            <div className="w-5 flex items-center">
              <Checkbox checked={isSelected} onClick={handleToggleSelect} className="mt-1" />
            </div>
            <a
              ref={listItemRef}
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex flex-col gap-2">
                  <p
                    ...
                    dangerouslySetInnerHTML={{ __html: highlightedContent.highlightedSubject ?? "(no subject)" }}
                  />
                  {highlightedContent.highlightedBody && (
                    <p
                      className="text-muted-foreground max-w-4xl text-xs md:text-sm truncate"
                      dangerouslySetInnerHTML={{ __html: highlightedContent.highlightedBody }}
                    />
                  )}
                </div>
              </div>
